...
- (void)viewWillAppear:(BOOL)animated {
    ...
    __emulation_paused = 0; // So when the cheat view is dismissed, the emulation resumes.
...
- (void)pauseMenu {
    ...
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"Paused", @"") message:nil delegate:self cancelButtonTitle:NSLocalizedString(@"Quit Game", @"") 
                                          otherButtonTitles:NSLocalizedString(@"Resume", @""), NSLocalizedString(@"Save State", @""), NSLocalizedString(@"Load State", @""), NSLocalizedString(@"Cheats", @""), nil];
    [alert show];
}

- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
    GBACheatViewController *cheatController = [GBACheatViewController new];
    cheatController.romPath = self.romPath;
    UINavigationController *nav = [[UINavigationController alloc] initWithRootViewController:cheatController];
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex {
    ...
    if (actionSheet.cancelButtonIndex == buttonIndex) { // Fixes crash when you cancel loading a savestate
        // I'm pretty sure I was the one who caused it to crash...
        __emulation_paused = 0;
        return;
    }
