...
extern unsigned short  *GBCBaseAddress;
extern int   __emulation_run;
extern volatile int __emulation_paused;

char gbc_save_filename[512];
char rom_filename[512];
	
int sndfreq = 44100;

int gbc_skip_next_frame = 1;
int save_sram = 0;
...
int EMU_MainLoop(void)
{
    ...
    if(fb.enabled) 
	{
		...
		memcpy(GBCBaseAddress, vidram, 160*144*2);
		updateGBCScreen();
...
int EMU_SaveSRAM(void)
{
    ...
    change_ext(rom_filename, sram_filename, ".sav");
...
int iphone_main_gbc(char* filename)
{
    ...
    printf("IPHONE STARTING GBC: %s\n", rom_filename);

	if( (!strcasecmp(rom_filename + (strlen(rom_filename)-4), ".svs")) )
	{
        printf("GBC In If statement, it is a saved game.");
		unsigned long pos;
		sprintf(gbc_save_filename, "%s", rom_filename);
	...
	if (strlen(gbc_save_filename) > 0)
	{
		EMU_LoadState(gbc_save_filename);
		gbc_save_filename[0] = '\0';
	}
